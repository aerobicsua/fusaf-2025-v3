...
class LiqPayService {
  ...
  // Deprecated: для зворотної сумісності - використовуйте createPaymentForm
  createPayment(params: any) {
    console.warn('createPayment is deprecated, use createPaymentForm instead');
    return this.createPaymentForm(params);
  }

  // Deprecated: для зворотної сумісності
  getPaymentUrl(): string {
    console.warn('getPaymentUrl is deprecated');
    return this.sandbox
      ? 'https://www.liqpay.ua/api/3/checkout'
      : 'https://www.liqpay.ua/api/3/checkout';
  }
